...
private repos), with protection enabled on `main`: pull requests required
before merging (0 required approvals, so solo work isn't blocked), no direct
pushes, no force-pushes, no branch deletion. The actual safety net — production
never gets touched if staging fails — lives in `.github/workflows/deploy.yml`
regardless; branch protection is the git-workflow discipline layered on top.
